			...
			foreach ($wordsArray as $key => $word) {
				...
				if (Str::contains($column, 'tCategory.') || Str::contains($column, 'tParentCat.')) {
					...
					if ((new Category)->isTranslatableAttribute($modelColumn)) {
						...
						$masterLocale = env('APP_LOCALE', config('app.fallback_locale'));
						...
						if ($jsonMethodsAreAvailable) {
							...
							$jsonColumn = 'LOWER(JSON_UNQUOTE(JSON_EXTRACT(' . $column . ', \'$.' . $locale . '\')))';
							$wordFilter = $jsonColumn . ' LIKE LOWER(?)';
							$wordBinding = '%' . $word . '%';
							
							if ($locale != $masterLocale) {
								$jsonColumn = 'LOWER(JSON_UNQUOTE(JSON_EXTRACT(' . $column . ', \'$.' . $masterLocale . '\')))';
								$masterLocaleWordFilter = $jsonColumn . ' LIKE LOWER(?)';
